...
# Install Qt5.7.0
# Dependencies: wget
mkdir /install_data && cd /install_data
...
7za x qt-everywhere-opensource-src-5.7.0.7z
mkdir -p /opt/Qt/Qt5.7.0
export QT5PREFIX=/opt/Qt/Qt5.7.0
./configure -prefix $QT5PREFIX -opensource -confirm-license -system-sqlite
make
make install
...
wget http://download.qt.io/official_releases/qtcreator/4.1/4.1.0/qt-creator-opensource-src-4.1.0.tar.xz

tar -xf qt-creator-opensource-src-4.1.0.tar.xz

# prepare the Makefile using qmake
mkdir qt-creator-build && cd qt-creator-build/ && qmake -r ../qt-creator-opensource-src-4.1.0/qtcreator.pro
